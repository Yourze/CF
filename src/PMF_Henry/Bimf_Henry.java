...
public class Bimf_Henry {
	...
	static double[] uAveRates = new double[DataInfo.userNumber];
	static double[] iAveRates = new double[DataInfo.itemNumber];
	...
	static void readTrainData(String trainPath, String split_Sign) throws NumberFormatException, IOException {
		...
		double sum = 0;
		double[] tempUTotRates = new double[DataInfo.userNumber]; 
		int[] tempUDegree = new int[DataInfo.userNumber];
		double[] tempITotRates = new double[DataInfo.itemNumber];
		int[] tempIDegree = new int[DataInfo.itemNumber];
		int index = 0;
		while (buffRead.ready()) {
			...
			index++;
			tempUTotRates[user] += rating;
			tempUDegree[user] ++;
			tempITotRates[item] += rating;
			tempIDegree[item] ++;
		...
		for(int i = 0; i < DataInfo.userNumber; i ++){
			if(tempUDegree[i] > 1e-6){
				uAveRates[i] = tempUTotRates[i] / tempUDegree[i];
			}//Of if 
		}//Of for i
		
		for(int i = 0; i < DataInfo.itemNumber; i ++){
			if(tempIDegree[i] > 1e-6){
				iAveRates[i] = tempITotRates[i] / tempIDegree[i];
			}//Of if 
		}//Of for i
	...
	static double predict(int paraUserId, int paraItemId) {
		...
		tempPredict += uAveRates[paraUserId] + iAveRates[paraItemId];
	...
	public static void update_one() {
		for (int i = 0; i < DataInfo.trainNumber; i++) {
			...
			double lambda2 = 1;
			double tmp = tempVary - lambda2 * (uAveRates[tempUserId] + iAveRates[tempItemId] - DataInfo.mean_rating);
			uAveRates[tempUserId] += DataInfo.alpha * tmp;
			iAveRates[tempItemId] += DataInfo.alpha * tmp;
		}//of for i
	}//Of update_one
